 ...
 * Começaremos com, talvez, a classe mais importante de todas: a String. 
 * Quanto mais conhecermos esta classe, mais fácil será o seu dia a dia enquanto desenvolvedor, 
 * pois trata-se de uma classe fundamental no desenvolvimento de projetos.
...
/**
 * Até este ponto não há nada de novo; temos a classe String que pertence ao pacote java.lang, 
 * e nome é uma referência.
 * Ao observarmos a classe TesteContas veremos que cc é uma referência, ContaCorrente é uma classe que 
 * representa um tipo, e ao lado direito temos o objeto.
 * */
...
/**  
Ou seja, em TesteString também estamos criando um objeto, Strings também são objetos. 
No entanto, não é preciso utilizar o new para criar este objeto. Isso ocorre para facilitar 
a vida do desenvolvedor, não precisamos utilizar o new todas as vezes que formos trabalhar com Strings, 
mas nada impede que você o faça. Portanto, as duas formas são funcionais, embora o segundo caso seja 
considerado uma má prática. A partir da primeira forma a máquina virtual consegue executar algumas otimizações, 
o que é impossível no segundo caso.
*/
...
Procuraremos a documentação oficial do Java 9, muito embora você possa utilizar a da versão 8 ou 7, já que a classe String se modificou muito pouco. Veremos que a estética da documentação é idêntica ao Javadoc gerado por nós. Estamos analisando especificamente a documentação sobre a String.
